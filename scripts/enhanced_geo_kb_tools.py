...
class IntelligentGeoKBSearcher:
    ...
    def _score_document(self, doc: Dict[str, Any]) -> float:
        ...
        title = doc.get("title", "")
        url = doc.get("url", "")
        keyword = doc.get("_source_keyword", "")
        ...
        if any(indicator in url.lower() for indicator in authority_indicators):
            score += 0.2
        
        # Boost for legal documents
        legal_indicators = ["resolución", "decreto", "ley", "normativa", "reglamento"]
        if any(indicator in title.lower() for indicator in legal_indicators):
            score += 0.15
